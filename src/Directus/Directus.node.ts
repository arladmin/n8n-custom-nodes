 ...
 */
			...
			if (resource === 'roles') {
				...
				if (operation == 'create') {
					try {
						const parametersAreJson = (this.getNodeParameter('jsonParameters', i) as boolean) ?? false;
			...
			if (resource === 'users') {
				...
				if (operation == 'create') {
					try {
						...
						if (parametersAreJson) {
							const bodyParametersJson = this.getNodeParameter('bodyParametersJson', i) as object | string;
							if (typeof bodyParametersJson == 'string') {
								body = JSON.parse(bodyParametersJson);
							} else {
								body = JSON.parse(JSON.stringify(bodyParametersJson));
			...
			if (resource === 'utils') {
				...
				if (operation == 'importFileData') {
					try {
						...
						if (sendBinaryData) {
							const item = items[i].binary as IBinaryKeyData;
							const binaryPropertyName = (this.getNodeParameter('binaryPropertyName', i) as string) ?? null;
							const binaryData = item[binaryPropertyName] as IBinaryData;
